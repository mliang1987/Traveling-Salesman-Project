...
class BranchAndBoundConfiguration:
  ...
  def expand(self, v):
    ...
    """
    # Check if the vertex is not in the path already, if appending it does not lead to a solution.
    if not (len(self._path) == self._N and v == self._path[0]) and v in self._path:
      raise ValueError
  ...
  def is_solution(self):
    ...
    return len(self._path) == self._N + 1
    ...
    while frontier:
      ...
      for v in range(self._N):
        ...
        if expanded_config.is_solution():
          ...
          upper_bound = min(upper_bound, expanded_config.get_cost()) \
              if upper_bound else expanded_config.get_cost()
